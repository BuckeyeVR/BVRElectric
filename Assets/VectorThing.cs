	...
	void Update () {
		if(didChange==true){
			...
			if(float.IsNaN(realVector.magnitude)==false){
				//transform.GetChild(0).localScale = new Vector3(0.15f, realVector.magnitude/1500f, 0.15f);
				transform.GetChild(0).localScale = new Vector3(0.15f, realVector.magnitude/300f, 0.15f);
			}
			else{
